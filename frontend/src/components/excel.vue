<template>
  <div>
    ...
    <section class="toolbar">
      <el-button type="info" round plain @click="createSheet">New CSV</el-button>
      <el-button type="info" round plain @click="importCSV">Import CSV</el-button>
      <input id="file" type="file" accept=".csv" hidden @change="loadCSV" />
      <el-button type="info" round plain @click="exportCSV">Export CSV</el-button>
      ...
      <el-input v-model="computeExp" placeholder="enter expression" clearable>
          ...
          <el-button slot="append" icon="el-icon-caret-right" @click="computeGrid"></el-button>
      </el-input>
    </section>
    <!-- sheet -->
    <section id="table">
...
export default {
  ...
  methods: {
    ...
    exportCSV() {
        ...
        dld.setAttribute("download", `output-${time.getHours()}H${time.getMinutes()}M.csv`);
        dld.click();
    },
    computeGrid(){
    ...
    mouseMove(ev) {
      ...
      //获取当前的x,y坐标
      this.mouseX =  ev.clientX + document.body.scrollLeft - document.body.clientLeft;
      this.mouseY =  ev.clientY + document.body.scrollTop - document.body.clientTop;
    },
  },
};
</script>
